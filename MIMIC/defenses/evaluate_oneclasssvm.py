...
def combine_results(output_directory):
    ...
    output_file = output_directory/"OneClassSVM_combined_results.csv"
...
def evaluate_oneclasssvm(output_directory):
    ...
    # scaler = RobustScaler()
    # clf = OneClassSVM(kernel='linear',  gamma='auto', coef0=0, tol=1e-5)
    ######################################################################################################################################
    # less (OE)
    ...
    train = np.load(data_dir/"mimic_train_all_benign_0.npy")
    train_x = train[:, :-1]
    train_y = train[:, -1]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Run MIMIC script: python evaluate_oneclasssvm.py <output_directory>",
